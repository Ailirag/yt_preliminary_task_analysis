...
- Токены/ключи только из переменных окружения (можно через `.env` в корне — грузится при старте,
  переменные окружения ОС имеют приоритет); никогда не выводить их значения в логи, отчёты, сообщения.
